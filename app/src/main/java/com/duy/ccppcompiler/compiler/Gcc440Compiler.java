...
public class Gcc440Compiler implements INativeCompiler {
    private static final String TAG = "GCCCompiler";
    private Context mContext;

    public Gcc440Compiler(Context context) {
    ...
    public ShellResult compile(File[] sourceFiles) {
        ...
        File gccDir = new File(internalDir, "android-gcc-4.4.0");
        final String sysPath = System.getenv("PATH");
        final File gccBinDir = new File(gccDir, "bin");
        final File armGccBinDir = new File(gccDir, "arm-eabi" + File.separator + "bin");
        final File libExecBinDir = new File(gccDir, "libexec/gcc/arm-eabi/" + GCCConstants.GCC_VERSION);

        String compilerPath = new File(gccBinDir, "arm-eabi-gcc").getAbsolutePath();
