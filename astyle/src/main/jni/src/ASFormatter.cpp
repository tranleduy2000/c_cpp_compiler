        ...
        while (!isLineReady) {
            ...
            if (currentChar == '{' || currentChar == '}') {
                ...
                bool isOpeningArrayBracket = (isBracketType(bracketType, ARRAY_TYPE)
                                              ...
                                              && !isBracketType(
                        (*bracketTypeStack)[bracketTypeStack->size() - 2], ARRAY_TYPE)
                );
                ...
                if (currentChar == '}') {
                    ...
                    if (isNonInStatementArray
                        && (!isBracketType(bracketTypeStack->back(),
                                           ARRAY_TYPE)    // check previous bracket
                            || peekNextChar() ==
            ...
            if (isPotentialOperator) {
                ...
                if (newHeader != NULL) {
                    ...
                    if (!isInPotentialCalculation) {
                        ...
                        if (find(assignmentOperators->begin(), assignmentOperators->end(),
                                 newHeader)
 ...
 */
    string ASFormatter::peekNextText(const string &firstLine, bool endOnEmptyLine /*false*/,
                                     bool shouldReset /*false*/) const {
 ...
 */
    ...
    void ASFormatter::updateFormattedLineSplitPoints(char appendedChar) {
        ...
        if (appendedChar == '{' || appendedChar == '}'
            ...
            || nextChar == '{' || nextChar == '}'
            || currentChar == '{' ||
            currentChar == '}')    // currentChar tests for an appended bracket
            return;
