 ...
 */
package com.duy.ide.editor.internal;
...
public class FastScroller {
    ...
    public FastScroller(Context context, EditText editText) {
        mEditText = editText;
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        if (mEditText.getWidth() > 0 && mEditText.getHeight() > 0) {
            onSizeChanged(mEditText.getWidth(), mEditText.getHeight(), 0, 0);
        }

        mState = STATE_NONE;
        refreshDrawableState();
        ta.recycle();
        setScrollbarPosition(mEditText.getVerticalScrollbarPosition());
    }
    ...
    public void draw(Canvas canvas) {
        ...
        if (mTrackDrawable != null) {
            ...
            mTrackDrawable.setBounds(trackLeft, halfThumbHeight,
                    trackLeft + trackWidth, mEditText.getHeight() - halfThumbHeight);
                ...
                switch (mPosition) {
                    ...
                    case View.SCROLLBAR_POSITION_LEFT:
                        left = Math.min(mThumbDrawable.getBounds().right + mThumbW,
                                mEditText.getWidth() - mOverlaySize);
                        break;
                }
                int top = Math.max(0,
                        Math.min(y + (mThumbH - mOverlaySize) / 2, mEditText.getHeight() - mOverlaySize));
    ...
    private void scrollTo(float position) {
        ...
        if (sections != null && sections.length > 1) {
            ...
            mEditText.setSelection(index + mListOffset);
        } else {
    ...
    boolean isPointInside(float x, float y) {
        ...
        switch (mPosition) {
            ...
            case View.SCROLLBAR_POSITION_DEFAULT:
            case View.SCROLLBAR_POSITION_RIGHT:
                inTrack = x > mEditText.getWidth() - mThumbW;
                break;
            case View.SCROLLBAR_POSITION_LEFT:
                inTrack = x < mThumbW;
                break;
