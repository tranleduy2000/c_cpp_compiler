...
unsigned int o_retry = 0;
unsigned int o_retrysleep = 0;
USHORT o_zero = 0;
...
/*
** CORKSCREW-2.0
** Copyright (C) Pat Padgett <agroman@agroman.net>
*/
...
int sock_connect (const char *hname, int port)
{
  ...
  {
    fprintf(stderr, "Failed to setsockopt: TCP_NODELAY.\n");
    return -1;
  }

  value = IPTOS_LOWDELAY;

  if (setsockopt(fd, SOL_IP, IP_TOS, &value, sizeof(value)) == -1)
  {
    fprintf(stderr, "Failed to setsockopt: IPTOS_LOWDELAY.\n");
    return -1;
  }
  ...
  {
    fprintf(stderr, "Failed to setsockopt: SO_RCVBUF.\n");
    return -1;
  }

  if (setsockopt(fd, SOL_SOCKET, SO_SNDBUF, &value, sizeof(value)) == -1)
  {
    fprintf(stderr, "Failed to setsockopt: SO_SNDBUF.\n");
    return -1;
  }
...
int httpProxyConnect (int count, char *args[])
{
  #ifdef DEBUG
  int x = 0;
  for(x; x < count; x++)
  {
    fprintf(stderr, "httpProxyConnect: args: %s \n",args[x]);
  }
  #endif
  ...
  if(csock == -1)
  {
    fprintf(stderr, "httpProxyConnect: Couldn't establish connection to proxy: %s\n", strerror(errno));
    exit(-1);
  }
      ...
      if (FD_ISSET(csock, &rfd))
      {
        ...
        else
        {
          ...
          else
          {
            ...
            fprintf(stderr, "httpProxyConnect: Proxy could not open connnection to %s: %s\n", desthost, descr);
            exit(-1);
          }
        }
      }
...
#endif
  rr = setsockopt (nnetfd, IPPROTO_TCP, TCP_NODELAY, &x, sizeof (x));
  if (rr == -1)
    holler ("nnetfd nodelay failed");		/* ??? */
  x = IPTOS_LOWDELAY;
  rr = setsockopt (nnetfd, SOL_IP, IP_TOS, &x, sizeof (x));
  if (rr == -1)
    holler ("nnetfd lowdelay failed");		/* ??? */
  x = 393216;
  rr = setsockopt(nnetfd, SOL_SOCKET, SO_RCVBUF, &x, sizeof (x));
  if (rr == -1)
    holler ("nnetfd set rcvbuf failed");		/* ??? */
  rr = setsockopt(nnetfd, SOL_SOCKET, SO_SNDBUF, &x, sizeof (x));
  if (rr == -1)
    holler ("nnetfd set sndbuf failed");		/* ??? */
#if 0
...
/* optarg, optind = next-argv-component [i.e. flag arg]; optopt = last-char */
  while ((x = getopt (argc, argv, "ae:g:G:H:hi:lno:p:q:rs:tuvw:zR:T:")) != EOF) {
...
/* do a full lookup [since everything else goes through the same mill],
   ...
   be useful, so we'll still pass o_nflag here instead of forcing numeric.  */
	...
	o_zero++;
	break;
    case 'R':				/* connection retrys */
	o_retry = atoi (optarg);
	if (o_retry <= 0)
	  bail ("invalid retry's number %s", optarg);
	break;
    case 'T':				/* connection retries */
	o_retrysleep = atoi (optarg);
	if (o_retrysleep <= 0)
	  bail ("invalid retry sleep time %s", optarg);
	o_retrysleep = o_retrysleep * 1000;
	break;
      default:
	errno = 0;
	bail ("nc -h for help");
...
Debug (("netfd %d errno %d from port %d to port %d", netfd, errno, ourport, curport))
...
/* sigh, this necessarily gets messy.  And the trailing \ characters may be
  ...
  holler ("\
	...
	-R num			connection retries number\n\
	-T milisecs		connection retry sleep time\n");
