 ...
 */
    void ASBeautifier::registerInStatementIndent(const string &line, int i, int spaceIndentCount_,
        ...
        if (isNonInStatementArray && !isInEnum && !bracketBlockStateStack->empty() &&
...
*/
    void
    ASBeautifier::registerInStatementIndentColon(const string &line, int i, int tabIncrementIn) {
        assert(line[i] == ':');
...
// Update the class variable indentCount.
	 ...
	 */
            ...
            // must check one less in headerStack if more than one header on a line (allow-addins)...
                 ...
                 && !(lineOpeningBlocksNum > 0 && lineOpeningBlocksNum <= lineClosingBlocksNum)
                 && (headerStack->size() > 2 &&
                     (*headerStack)[headerStack->size() - 3] != &AS_OPEN_BRACKET)
                 && shouldIndentBrackettedLine)
            --indentCount;
    ...
    void ASBeautifier::adjustObjCMethodCallIndentation(const string &line_) {
        ...
        } else    // align keywords instead of colons
        {
            if (isInObjCMethodCallFirst) {
                ...
                if (spaceIndentCount <
                    keywordIndentObjCMethodAlignment + bracketPosObjCMethodAlignment)
                    spaceIndentCount += keywordIndentObjCMethodAlignment;
            }
        }
    }
        ...
        for (size_t i = 0; i < line.length(); i++) {
            ...
            if (!(isInComment || isInLineComment)
                ...
                if (!isInQuote) {
                    ...
                    else if (g_preprocessorCppExternCBracket == 2 &&
            ...
            if (ch == '(' || ch == '[' || ch == ')' || ch == ']') {
                if (ch == '(' || ch == '[') {
                    ...
                    if (currentHeader != NULL)
                        registerInStatementIndent(line, i, spaceIndentCount, tabIncrementIn,
                                                  minConditionalIndent/*indentLength*2*/, true);
                    else
                        registerInStatementIndent(line, i, spaceIndentCount, tabIncrementIn, 0,
                                                  true);
                } else if (ch == ')' || ch == ']') {
                    if (ch == ']')
                        --blockParenCount;
